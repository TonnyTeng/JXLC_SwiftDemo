...
struct Proxy{
    ...
    var imageType:ImageType = .SERVICE
    ...
    init(type:ImageType,array:[String]){
    
        imageType = type
        if imageType == .SERVICE
        {
            imageArray = array.map({ (urlStr) -> ImageSource in
                return ImageSource.SERVICE(url: URL(string: urlStr)!)
            })
        }
        else
        {
            imageArray = array.map({ (name) -> ImageSource in
                return ImageSource.LOCAL(name: name)
            })
        }
    }
}
...
private let PageControllMargin:CGFloat = 15
private let PageControllWidth:CGFloat = 2
...
extension PageControlAlimentProtocol where Self : UIView{
    ...
    func relayoutPageControl(pageControl: UIPageControl)
    {
        if pageControl.isHidden == false
        {
            ...
            let pageW = CGFloat(pageControl.numberOfPages - 1) * (self.pageControlPointSpace + PageControllWidth) + PageControllWidth
            var pageX:CGFloat = 0
            ...
            case .RightBottom:
                pageX = bounds.width - pageW - PageControllMargin
            case .LeftBottom:
                pageX = bounds.origin.x + PageControllMargin
            }
